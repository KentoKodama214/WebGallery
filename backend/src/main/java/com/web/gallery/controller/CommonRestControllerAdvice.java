...
public class CommonRestControllerAdvice {
  ...
  public ResponseEntity<BadRequestResponse> handleBadRequestException(
    ...
    return new ResponseEntity<BadRequestResponse>(
        BadRequestResponse.of(exception), HttpStatus.BAD_REQUEST);
  }
  ...
  public ResponseEntity<ErrorResponse> handleForbiddenAccountException(
  ...
  public ResponseEntity<ErrorResponse> handleFileDuplicateException(
      FileDuplicateException exception) {
  ...
  public ResponseEntity<ErrorResponse> handlePhotoNotAdditableException(
      PhotoNotAdditableException exception) {
  ...
  public ResponseEntity<ErrorResponse> handleInsertFailedException(
      RegistFailureException exception) {
    ErrorResponse errorResponse = ErrorResponse.of(exception, HttpStatus.CONFLICT);
  ...
  public ResponseEntity<ErrorResponse> handleUpdateFailureException(
      UpdateFailureException exception) {
    ErrorResponse errorResponse = ErrorResponse.of(exception, HttpStatus.CONFLICT);
  ...
  /**
   * リクエストボディのバインド・バリデーション・パースに失敗したときに制御するExceptionHandler
   *
   * <p>（{@code @Valid}付きボディの検証失敗、不正なJSON、必須パラメータ欠落、型不一致、アップロードサイズ超過）
   *
  ...
  public ResponseEntity<BadRequestResponse> handleRequestBindingException(Exception exception) {
    log.info(
        "Invalid request. ({}: {})", exception.getClass().getSimpleName(), exception.getMessage());

    BadRequestException badRequestException =
        (BadRequestException) ErrorEnum.INVALID_INPUT.toException();
    return new ResponseEntity<BadRequestResponse>(
        BadRequestResponse.of(badRequestException), HttpStatus.BAD_REQUEST);
  }

  /**
   * 個別のExceptionHandlerで捕捉されない予期しないDBアクセスエラーが発生したときに制御するExceptionHandler
   *
   * <p>スタックトレース等の内部情報を含まない一般的なエラーレスポンスに変換する安全網として機能する
   *
  ...
  public ResponseEntity<ErrorResponse> handleDataAccessException(DataAccessException exception) {
    ...
    ErrorResponse errorResponse =
        ErrorResponse.of(systemException, HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  /**
   * 上記いずれのExceptionHandlerでも捕捉されない予期しない例外が発生したときに制御するExceptionHandler
   *
   * <p>スタックトレース等の内部情報を含まない一般的なエラーレスポンスに変換する最終的な安全網として機能する
   *
