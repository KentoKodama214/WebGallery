...
public class PhotoListGetResponse {
  ...
  /**
   * PhotoPageModelからPhotoListGetResponseを生成する
   *
   * <p>DB側で既にページング済みの結果をそのまま変換する
   *
  ...
  public static PhotoListGetResponse from(PhotoPageModel photoPageModel) {
    List<PhotoListResponse> photoListResponseList =
        photoPageModel.getPhotoModelList().stream().map(PhotoListResponse::from).toList();
