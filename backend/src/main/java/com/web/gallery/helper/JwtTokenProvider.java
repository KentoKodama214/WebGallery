...
public class JwtTokenProvider {
  ...
  /**
   * 起動時にJWTシークレットキーの長さを検証する
   *
   * <p>HS256では256bit以上の鍵長が必須であり、短い鍵だと総当たりで署名偽造が可能になる
   *
  ...
  void validateSecret() {
    ...
    if (secret == null || secret.getBytes(StandardCharsets.UTF_8).length < MIN_SECRET_BYTE_LENGTH) {
      throw new IllegalStateException("JWTシークレットキー（app.jwt.secret）は256bit（32バイト）以上である必要があります");
    }
  }
  ...
  public String generateAccessToken(AccountPrincipal principal) {
    Date now = new Date();
    Date expiry =
        new Date(now.getTime() + jwtConfig.getAccessTokenExpirationMinutes() * 60 * 1000L);
