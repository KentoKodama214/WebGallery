...
public class Photo {
	...
	private Photo(AccountNo accountNo, PhotoNo photoNo, PhotoDetailModel detail, ImageFilePath imageFilePathForDelete) {
	 ...
	 * @return							{@link Photo}
	 */
	public static Photo forRegist(PhotoDetailModel requestDetail, PhotoNo newPhotoNo, ImageFilePath assignedImageFilePath) {
		PhotoDetailModel detail = requestDetail.toBuilder()
				.photoNo(newPhotoNo)
				.imageFilePath(assignedImageFilePath)
				.build();
	...
	public static Photo forUpdate(PhotoDetailModel requestDetail) {
		Photo photo = new Photo(requestDetail.getAccountNo(), requestDetail.getPhotoNo(), requestDetail, null);
		photo.updateTags(requestDetail.getPhotoTagModelList());
		return photo;
	}
	 ...
	 * @return							{@link Photo}
	 */
	public static Photo forDelete(AccountNo accountNo, PhotoNo photoNo, ImageFilePath imageFilePathForDelete) {
	...
	/**
	 * 写真の詳細情報を取得する<p>
	 * {@link #forDelete}で生成したPhotoでは呼び出せない（detailを保持しないため）
	 *
