...
public class ImageFileValidationPolicy {
  ...
  /**
   * 画像ファイルの先頭バイト列（マジックバイト）が、既知の画像フォーマット（JPEG・PNG・GIF・WebP）のシグネチャと一致するかどうかを判定する
   *
   * <p>偽装されたContent-Typeや拡張子を見破るため、実際のバイナリ内容を検証する。ファイルの読み込みに失敗した場合はfalseを返す
   *
   * @param imageFile {@link ImageFile}
  ...
  public Boolean isValidSignature(ImageFile imageFile) {
    ...
    return matchesAt(header, 0, JPEG_SIGNATURE)
        ...
        || (matchesAt(header, 0, RIFF_SIGNATURE)
