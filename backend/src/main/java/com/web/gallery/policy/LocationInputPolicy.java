...
public class LocationInputPolicy {
   ...
   *
   * <p>ロケーション番号（既存マスタからの選択）が指定されている場合は、名称・緯度・経度の入力は問わない（サーバ側は
   * ロケーション番号のみを信頼するため）。ロケーション番号が未指定でロケーション名（新規入力）が指定されている場合は、
   * 緯度・経度の両方が指定されていることを要求する。いずれも未指定の場合はロケーション未設定として許可する
   *
  ...
  public Boolean isValid(
      LocationNo locationNo, LocationName locationName, GeoLocation geoLocation) {
    boolean isExistingSelection = locationNo != null && locationNo.value() > 0;
    if (isExistingSelection) {
      return true;
    }

    boolean isNewInput = locationName != null;
    if (!isNewInput) {
      return true;
    }
