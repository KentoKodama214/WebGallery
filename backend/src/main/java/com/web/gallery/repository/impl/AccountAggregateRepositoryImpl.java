...
/**
 * アカウント集約（{@link Account}）を永続化するRepositoryの実装クラス<p>
 * お気に入り・写真タグ・写真マスタ・リフレッシュトークン・アカウントの各テーブルへの永続化を、
 * アカウント削除というユースケース単位で整合性のある1操作としてまとめる。他のRepositoryには依存せず、
 * Mapperを直接操作する
 */
@Slf4j
@Repository
...
public class AccountAggregateRepositoryImpl implements AccountAggregateRepository {
	...
	public void delete(Account account) {
		...
		PhotoNoList deletedPhotoNoList = PhotoNoList.from(photoMstMapper.deletePhotosByAccountNo(accountNo).stream()
