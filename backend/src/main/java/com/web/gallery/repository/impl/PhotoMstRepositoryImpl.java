...
public class PhotoMstRepositoryImpl implements PhotoMstRepository {
   ...
   */
  @Override
  public void regist(PhotoDetailModel photoDetailModel, ImageFilePath filePath, PhotoNo newPhotoNo)
      throws GalleryException {
    PhotoMst photoMst =
        PhotoMst.fromForRegist(photoDetailModel, filePath.value(), newPhotoNo.value());
    ...
    try {
      ...
      log.warn(
          ...
          photoDetailModel.getAccountNo().value(),
          newPhotoNo.value(),
          e);
  ...
  public void update(PhotoDetailModel photoDetailModel) throws GalleryException {
    PhotoMstCondition condition =
        PhotoMstCondition.byAccountAndPhoto(
            photoDetailModel.getAccountNo().value(), photoDetailModel.getPhotoNo().value());
    ...
    if (photoMstMapper.update(condition, target) < 1) {
      log.warn(
          ...
          photoDetailModel.getAccountNo().value(),
          photoDetailModel.getPhotoNo().value());
  ...
  public void delete(PhotoDeleteModel photoDeleteModel) throws GalleryException {
    PhotoMstCondition condition =
        PhotoMstCondition.byAccountAndPhoto(
            photoDeleteModel.getAccountNo().value(), photoDeleteModel.getPhotoNo().value());
    PhotoMstUpdateTarget target = PhotoMstUpdateTarget.forDelete(photoDeleteModel);

    if (photoMstMapper.update(condition, target) < 1) {
      log.warn(
          "PhotoMst: Delete Failed (AccountNo: {}, PhotoNo: {})",
          photoDeleteModel.getAccountNo().value(),
          photoDeleteModel.getPhotoNo().value());
  ...
  public PhotoNoList deleteAndGetUndeletedPhotoNosByAccountNo(AccountNo accountNo) {
    return PhotoNoList.from(
        photoMstMapper.deletePhotosByAccountNo(accountNo.value()).stream()
