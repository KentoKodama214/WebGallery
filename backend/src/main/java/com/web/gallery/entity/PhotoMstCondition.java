...
public class PhotoMstCondition {
  ...
  public static PhotoMstCondition byAccountAndPhoto(Long accountNo, Long photoNo) {
    return PhotoMstCondition.builder().accountNo(accountNo).photoNo(photoNo).build();
  }

  /**
   * アカウント番号・写真番号による抽出条件を生成する（削除済みを除外）
   *
   * <p>更新・削除対象のWHERE句に使用し、削除済みの写真への二重更新・二重削除を防ぐ
   *
   * @param accountNo アカウント番号
   * @param photoNo 写真番号
   * @return {@link PhotoMstCondition}
  ...
  public static PhotoMstCondition forCount(Long accountNo) {
    return PhotoMstCondition.builder().accountNo(accountNo).isDeleted(false).build();
  }
}
