...
### EXIF情報（撮影メタデータ）のサーバー側検証

写真新規一括登録（`POST /api/v1/accounts/{id}/photos`）の焦点距離・F値・シャッタースピード・ISOは、
クライアント申告値（フォーム入力値）をそのまま信頼せず、アップロードされた画像ファイルの実バイナリから
`PhotoExifExtractor`（`helper/`）がEXIFを抽出し、項目ごとに抽出値を優先して採用する
（`PhotoExifDataMergePolicy#merge`）。クライアントは任意の値を送信できるため、申告値のみに依拠すると
実際の撮影データと異なる値を表示させられる（データ偽装）ため。

- 抽出はJPEGのEXIFのみに対応する（PNG等EXIFを保持しない形式では非対応）。抽出できなかった項目・
  形式は、その項目のみクライアント申告値にフォールバックする（マニュアル入力機能を維持するため）。
