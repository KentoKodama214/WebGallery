...
パスワード変更（`PUT /api/v1/accounts/{id}` で新しいパスワードを指定。本人確認はリクエストボディの
`currentPassword`）とアカウント削除（`DELETE /api/v1/accounts/{id}`。本人確認は `X-Reauth-Password`
ヘッダー）は、アクセストークンの有効性に加えて現在のパスワードによる本人確認を必須とする
...
エントリ数が上限（10万）に達した場合は、期限切れ→直近失敗が古い順に間引く（全消去はしない。
全消去するとロックアウト中の全アカウントが一斉解除され、失敗枠を膨らませて悪用されうるため）。
...
`DELETE /api/v1/accounts/{id}` の本人確認情報は `X-Reauth-Password` ヘッダーで受け取る。
DELETE のボディを破棄する中継経路（一部の CDN・リバースプロキシ・WAF）があっても
確実に透過させるため、ボディではなくヘッダーを用いる。
