...
## 1. 事前準備と情報収集
   ...
   - `development` の場合は PR を作成できないため警告して終了する。
   - `git log origin/development..HEAD --oneline` で差分コミット一覧を取得する。
   - `git diff origin/development...HEAD` でコードの全差分を取得する。
...
## 2. ドキュメント整合性チェック（必須）
   ...
   - 該当する変更があるのに対応ドキュメントが差分に含まれていない場合は、このスキル内でドキュメントを更新し、更新内容をユーザーに提示した上でそのまま変更に含めて次に進む（`doc/modulith/`配下などツールによる自動生成物は対象外）。確認待ちはしない。
   - 判断に迷う場合（更新要否の判定自体が困難な場合）のみ、自己判断で済ませずユーザーに確認する。
...
## 5. 実行
   ...
   - 生成した PR タイトルと本文を提示した上で、確認を待たずに `gh pr create --title "..." --body "..."` を実行する。
