---
name: commit-push
description: Githubへコミット・プッシュを実行するスキル。「コミットして」「プッシュして」「/commit-push」などの指示があった時に使用する。development/staging/masterブランチの保護（PRマージのみで更新）、メッセージ付きのコミットまで行う
---
...
## 1. 事前チェック
   ...
   - 現在のブランチ名を取得する。`development`・`staging`・`master` のいずれかの場合、これらはPRマージのみで更新するブランチであり直接コミット・プッシュできない旨を警告し、`/create-branch` での作業ブランチ作成を促して処理を停止する。
